...
def WindowsBoxes(sTitle, sFileName, metaType, year=''):
    ...
    try:
		...
		xbmc.executebuiltin('RunScript(plugin.video.themoviedb.helper,add_query=$INFO[Container(90050).ListItem.Label],type=tv,call_auto=1134))' % sFileName)

                return
            elif metaType == '1':
                DIALOG.VSinfo('Lancement de ExtendInfo')
		xbmc.executebuiltin('RunScript(plugin.video.themoviedb.helper,add_query=$INFO[Container(90050).ListItem.Label],type=movie,call_auto=1134))' % sFileName)
                return
    except:
        pass
